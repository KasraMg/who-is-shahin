      ...
      <section className='px-36 relative z-20  sm-x2:overflow-hidden  pb-[70px] lg:px-10 pt-28 md:pt-10'>
        ...
        <div className='text-9xl md:text-6xl md:text-center   sm-x2:mt-8 sm-x3:!text-4xl  md:w-full w-max relative text-white font-swap font-extrabold'>
          <span className='z-20 relative sm-x2:whitespace-nowrap'>{t('topbar.title')}</span>
           <img className={ ` ${context.language=='fa' ? 'md:left-40 sm-x3:!left-0  left-10' : ' md:right-40 sm-x3:!right-0  right-2'} absolute top-4 md:top-0 w-36  z-10   `} src="https://demo.themecon.net/onlyme/html/images/hero-title-pettern-bg.png" alt="" />
           </div>
        ...
        <div className={` ${context.language =='fa' ? ' gap-3 flex items-center justify-start sm:justify-center' : null}  pt-12 md:text-center  `}>
        ...
        <img src="/Images/shomal2.jpg" className={`${context.language=='fa' ? 'left-[220px] lg:left-28':'right-[220px] lg:right-28'} me-image absolute   w-[420px] -top-[74px]  md:hidden`}  alt="" />
        <img src="https://demo.themecon.net/onlyme/html/images/dot-texture.png" className={`${context.language=='fa' ? ' md:!left-[45%]  lg:left-5 left-[97px] ':' md:!right-[45%]  lg:right-5 right-[97px]'} dot-bg  md:z-[-1] md:-top-20 top-32 absolute`} alt="" />
      </section>
      <div className={` ${context.language=='fa' ? 'mr-auto ml-10' : ' ml-auto '} flex gap-5 w-11 sm-x2:relative  sm-x2:top-12  pb-10 flex-col text-2xl text-white` }>
